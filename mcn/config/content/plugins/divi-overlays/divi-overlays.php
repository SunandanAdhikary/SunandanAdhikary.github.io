...
function divi_overlay_config( $hook ) {
	...
	wp_register_style( 'jquery_ui_css', DOV_PLUGIN_URL . 'overlay-effects/css/admin/jquery-ui-1.12.1.custom.css', array(), '1.12.1', 'all' );
	
	wp_register_style( 'divi-overlays-admin', DOV_PLUGIN_URL . 'overlay-effects/css/admin/admin.css', array(), '1.0.0', 'all' );
	wp_register_script( 'divi-overlays-admin-functions', DOV_PLUGIN_URL . 'overlay-effects/js/admin/admin-functions.js', array( 'jquery', 'jquery-ui-core', 'jquery-ui-slider', 'divi-overlays-select2' ), '1.0.0', true );
}
...
function showOverlay( $overlay_id = NULL ) {
	...
	<div id="divi-overlay-container-<?php echo $overlay_id;?>" class="overlay-container">
		...
		data-scrolltop="" data-cookie="<?php print $close_cookie ?>" data-enableajax="<?php print $enableajax ?>" data-contentloaded="0">
			...
			<a href="javascript:;" class="overlay-close overlay-customclose-btn-<?php echo $overlay_id ?>"><span class="<?php if ( $customizeclosebtn[0] == 1 ) { ?>custom_btn<?php } ?>">&times;</span></a>
			<?php } ?>
...
function divioverlays_refreshdivi() {
	...
	if ( isset( $_GET['divioverlays_id'] ) && isset( $_GET['action'] ) && $_GET['action'] == 'divioverlays_getcontent' ) {
		...
		dov_disable_divi_static_file();
	}
}

function dov_disable_divi_static_file() {
...
if ( !is_divi_builder_enabled() ) {
	...
	add_action( 'wp_footer', 'dov_disable_divi_static_file', 21 );
	add_action( 'wp_footer', 'custom_js_function', 2 );
}
